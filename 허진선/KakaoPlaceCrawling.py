...
class PlaceInfoScraper:
    ...
    def get_place_info(self, address):  # 각 장소별 리뷰 데이터를 크롤링
        ...
        for review in reviews:
            ...
            user_rating = int(rating_per) / 20  # 유저별 별점
            written_at.append(time_write)
            star_rate.append(user_rating)

        ar = np.array([written_at, star_rate]).T
        df = pd.DataFrame(ar, columns=["written_at", "star_rate"]);
        return img_url, df


class PlaceListScraper:
    def __init__(self):
        self.code_dic = {"문화시설": "CT1", "관광명소": "AT4", "음식점": "FD6", "카페": "CE7"}  # 카테고리 코드
    ...
    def get_place_list(self, code_key):
        ...
        for i in range(1, self.num_x + 1):  # 지도를 사각형으로 나누면서 데이터 받아옴
            ...
            for j in range(1, self.num_y + 1):
                ...
                each_result = self.search_places_in_range(self.code_dic[code_key], self.start_x, initial_start_y, end_x, end_y)
                overlapped_result.extend(each_result)
                initial_start_y = end_y
            self.start_x = end_x

        place_list = list(map(dict, OrderedDict.fromkeys(tuple(sorted(d.items())) for d in overlapped_result))) # 중복값 제거
        ...
        ar = np.array([place_name, x, y, road_address, place_url]).T
        df = pd.DataFrame(ar, columns=["place_name", "x", "y", "road_address", "place_url"]);
        return df


if __name__ == "__main__":
    listScraper = PlaceListScraper()
    place_df = listScraper.get_place_list("문화시설")
    print(place_df)

    infoScraper = PlaceInfoScraper()
    for idx, ser in place_df.iterrows():
        print(ser["place_url"])
        ser["img_url"], ser["review"] = infoScraper.get_place_info(ser["place_url"])